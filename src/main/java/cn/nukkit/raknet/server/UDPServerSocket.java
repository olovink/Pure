...
public class UDPServerSocket extends ChannelInboundHandlerAdapter {
    ...
    public UDPServerSocket(MainLogger logger, int port, String interfase) {
        this.logger = logger;
        try {
            ...
            bootstrap
                    ...
                    .handler(this);
            channel = bootstrap.bind(interfase, port).sync().channel();
        } catch (Exception e) {
            this.logger.critical("**** FAILED TO BIND TO " + interfase + ":" + port + "!");
